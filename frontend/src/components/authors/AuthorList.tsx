  ...
  const columns = [
    {
      header: 'Name',
      accessor: '',
      render: (data: Author) => <Link to={`/author/${data?.id}`}>{data?.name}</Link>,
    },
    {
      header: 'No, of books',
      accessor: 'books',
  ...
  return (
    <div className="mx-10 mt-5">
      <div className="flex flex-col sm:flex-row sm:justify-between gap-2 my-2">
        ...
        <Button type="submit" variant={'teal'} onClick={() => navigate('/authors/add')}>
          Add Author
        </Button>
      </div>
      <Table data={authorData} columns={columns} loading={loading} error={error != null} />
    </div>
  );
}
