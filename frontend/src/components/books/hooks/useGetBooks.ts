...
export const useGetBooks = () => {
  const { debouncedSearchText: title, onInputSearch } = useDebouncedSearchText(100);

  const { authorOptions } = useGetAuthorOptions();

  const [authors, setAuthors] = useFilterByOptions([DEFAULT_OPTION]);
  ...
  const { loading, error, data, refetch } = useQuery(GET_BOOKS, {
    variables: {
      title,
      authorIds: authors?.map((a) => a.value).filter((v) => v !== ''),
      yearPublished,
      noOfPages,
    },
